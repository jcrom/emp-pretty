        ...
        (function jspretty__tokenize() {
            var a              = 0,
                ...
                blockinsert    = function jspretty__tokenize_blockinsert() {
                    ...
                    next = next.slice(0, 4);
                    if (next === "else" && ltoke === "}" && depth[lengtha - 1] === "if" && token[begin[lengtha - 1]] !== "x{") {
                        return;
                    }
                    if (ltoke === ";" && token[g - 1] === "x{") {
                        name = token[begin[g - 2] - 1];
                        if (token[g - 2] === "do" || (token[g - 2] === ")" && "ifforwhilecatch".indexOf(name) > -1)) {
                            tokenpop();
                            ltoke = "x}";
                            ltype = "end";
                            tokenpush(false, 0);
                ...
                word           = function jspretty__tokenize_word() {
                    ...
                    if (output === "else") {
                        ...
                        if (token[lengtha - 2] === "x}") {
                            if (token[lengtha - 1] === "else") {
                                if (depth[lengtha - 2] !== "if" && depth[lengtha - 2] !== "else") {
                                    brace.pop();
                                    token.splice(lengtha - 2, 0, "x}");
                                    types.splice(lengtha - 2, 0, "end");
                                    lines.splice(lengtha - 2, 0, 0);
                                    depth.splice(lengtha - 2, 0, "if");
                                    begin.splice(lengtha - 2, 0, begin[begin[begin[lengtha - 2] - 1] - 1]);
                                    lengtha = lengtha + 1;
                                    depthlist.splice(depthlist.length - 2, 1);
                                    depthlist[depthlist.length - 1][1] = lengtha - 1;
                                } else if (token[lengtha - 3] === "x}" && pdepth[0] !== "if" && depth[depth.length - 1] === "else") {
                                    elsefix();
                                } else if (token[lengtha - 3] === "}" && depth[lengtha - 3] === "if" && pdepth[0] === "if" && token[pdepth[1] - 1] !== "if" && token[begin[lengtha - 2]] === "x{") {
                                    //fixes when "else" is following a block that isn't "if"
                                    elsefix();
                                }
                            } else if (token[lengtha - 1] === "x}" && depth[depth.length - 1] === "if") {
                                elsefix();
                            }
                        }
